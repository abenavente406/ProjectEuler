 ...
 */
public class Problem8 implements Exercise {
    @Override
    public void run() {
        ...
        for (int i = 0; i < largeNum.length() - 5; i++) {
            long tmpProduct = 1;
            for (int j = i;  j < i + 5; j++) {
                tmpProduct *= Integer.parseInt(largeNum.substring(j, j+1));
            }
            greatestProduct = Math.max(greatestProduct, tmpProduct);
        }
